...
type OrderQuery struct {
	...
	withUser    *UserQuery
	withProduct *RechargeProductQuery
...
func (_q *OrderQuery) QueryProduct() *RechargeProductQuery {
	query := (&RechargeProductClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *OrderQuery) WithProduct(opts ...func(*RechargeProductQuery)) *OrderQuery {
	query := (&RechargeProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withProduct; query != nil {
		if err := _q.loadProduct(ctx, query, nodes, nil,
			func(n *Order, e *RechargeProduct) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrderQuery) loadProduct(ctx context.Context, query *RechargeProductQuery, nodes []*Order, init func(*Order), assign func(*Order, *RechargeProduct)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Order)
	for i := range nodes {
	...
	query.Where(rechargeproduct.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
