...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	rechargeProductService := service.NewRechargeProductService(rechargeProductRepository, rechargeProductCache)
	...
	oidcSessionCache := repository.NewSSOSessionCache(redisClient, cryptoService)
	oidcssoService := service.ProvideOIDCSSOService(settingService, userRepository, authService, oidcSessionCache)
	ssoHandler := handler.NewSSOHandler(oidcssoService, settingService)
	paymentCache := repository.NewPaymentCache(redisClient)
	paymentService := service.NewPaymentService(orderRepository, userRepository, rechargeProductRepository, settingService, billingCacheService, paymentCache)
	...
	rateLimitCache := repository.NewRateLimitCache(redisClient)
	authRateLimitService := service.NewAuthRateLimitService(rateLimitCache)
	authRateLimitMiddleware := middleware.NewAuthRateLimitMiddleware(authRateLimitService)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, authRateLimitMiddleware, apiKeyService, subscriptionService)
	httpServer := server.ProvideHTTPServer(configConfig, engine)
	tokenRefreshService := service.ProvideTokenRefreshService(accountRepository, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, configConfig)
	orderCleanupService := service.ProvideOrderCleanupService(orderRepository)
	v := provideCleanup(client, redisClient, tokenRefreshService, pricingService, emailQueueService, billingCacheService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, oidcssoService, orderCleanupService)
